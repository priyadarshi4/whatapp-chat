...
// GET paginated with reply population
...
router.post('/', auth, async (req, res) => {
  try {
    const { chatId, content, type = 'text', mediaUrl, songData, unlockAt, replyTo } = req.body;
    const isUnlocked = !unlockAt || new Date(unlockAt) <= new Date();
    const msg = new Message({
      chatId, senderId: req.userId, content, type, mediaUrl, songData,
...
router.delete('/:id', auth, async (req, res) => {
  try {
    const msg = await Message.findOne({ _id: req.params.id, senderId: req.userId });
    if (!msg) return res.status(404).json({ error: 'Not found' });
    msg.isDeleted = true;
